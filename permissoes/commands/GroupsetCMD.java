...
public class GroupsetCMD implements CommandExecutor {
	...
	public GroupsetCMD(PermissionsManager manager) {
		...
		if (cmd.getName().equalsIgnoreCase("groupset")) {
			...
			if (args.length != 2) {
				sender.sendMessage(ChatColor.RED
						+ "Use: /groupset [nick] [grupo]");
				return true;
			}
			if (args[0].length() > 16) {
				sender.sendMessage(ChatColor.RED
						+ "Nome longo, use até 16 letras");
				return true;
			}
				...
				if (manager.getPlayerGroup(player) == Group.ADMIN) {
					if (group == Group.ADMIN || group == Group.DONO) {
						player.sendMessage(ChatColor.RED
								+ "Você não pode alterar o grupo desse player");
						return true;
					}
				}
			}
					...
					public void run() {
						...
						if (senderr instanceof Player) {
							...
							if (manager.getPlayerGroup(uuid) == Group.DONO
								...
								senderr.sendMessage(ChatColor.RED
										+ "Você não pode alterar o grupo de um dono");
								return;
							}
						}
						if (group == manager.getPlayerGroup(uuid)) {
							senderr.sendMessage(ChatColor.RED
									+ "O jogador já esta no grupo "
									+ group.toString());
							return;
						}
						...
						senderr.sendMessage(ChatColor.WHITE + argss[0]
								+ " setado como: " + group.toString()
